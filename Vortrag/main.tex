...
\begin{columns}
		...
		%\vspace{-40pt}
			\begin{itemize}
				\item Create Point Clouds on the fly from RGB and Depth images
				\item Compute point cloud normals
				\item Apply bilateral filter
				\item Option to read trajectory ground truth  with TF
...
\includegraphics[scale=0.8]{dia1.png}
 ...
 \hspace*{0.3cm}
       \begin{subfigure}[h]{0.45\textwidth}
        \includegraphics[scale=0.23]{truncation.png}\\
        \\
...
\begin{columns}
		...
		\vspace{-30pt}
	\begin{align*}
D &\gets \frac{W D+ \omega_{n+1}d_{n+1}}{W+\omega_{n+1}} \\
W &\gets W + \omega_{n+1}
\end{align*}
\end{columns}
...
\begin{columns}
		...
		%\vspace{-30pt}
			\begin{itemize}
			    ...
			    \item We search transformation from last camera position\\
			    given in twist coordinates $\xi = (\underbrace{v_1, v_2, v_3}_{\text{linear velocity}}, \underbrace{w_1, w_2, w_3}_{\text{angular velocity}})$
			    \item Minimize the squared distance error $E(\xi)$ between points and surface %$E = \sum\limits_\mathbf{x} \Psi_{R, t}()$  
		\end{itemize}
	\end{columns}
}
...
\begin{columns}
                ...
                %\vspace{-30pt}
                        \begin{itemize}
                            \item We search transformation from last camera position\\
                            given in twist coordinates $\xi = (\underbrace{v_1, v_2, v_3}_{\text{linear velocity}}, \underbrace{w_1, w_2, w_3}_{\text{angular velocity}})$
                            \item Minimize the squared distance error $E(\xi)$ between points and surface
...
\begin{columns}
                \column{0.45\textwidth}
                        \begin{figure}
                                \includegraphics[scale=0.25]{camera_estimation6.png}
                        \end{figure}
                \column{0.55\textwidth}
                %\vspace{-30pt}
                        \begin{itemize}
...
\begin{columns}
                \column{0.45\textwidth}
                        \begin{figure}
                                \includegraphics[scale=0.25]{camera_estimation7.png}
                        \end{figure}
                \column{0.55\textwidth}
                %\vspace{-30pt}
                        \begin{itemize}
...
\begin{columns}
                \column{0.45\textwidth}
                        \begin{figure}
                                \includegraphics[scale=0.25]{camera_estimation8.png}
                        \end{figure}
                \column{0.55\textwidth}
                %\vspace{-30pt}
                        \begin{itemize}
...
\begin{columns}
                \column{0.45\textwidth}
                        \begin{figure}
                                \includegraphics[scale=0.25]{camera_estimation9.png}
                        \end{figure}
                \column{0.55\textwidth}
                %\vspace{-30pt}
                        \begin{itemize}
                            ...
                            \begin{itemize}
                                  \item Linearize Transformation
                                  \item Solve linear system $\nabla E = 0 \to \xi$
                                  \item \textbf{Apply transformation to camera}
...
\begin{columns}
                \column{0.45\textwidth}
                        \begin{figure}
                                \includegraphics[scale=0.25]{camera_estimation10.png}
                        \end{figure}
                \column{0.55\textwidth}
                %\vspace{-30pt}
                        \begin{itemize}
...
\begin{columns}
                \column{0.45\textwidth}
                        \begin{figure}
                                \includegraphics[scale=0.25]{camera_estimation11.png}
                        \end{figure}
                \column{0.55\textwidth}
                %\vspace{-30pt}
                        \begin{itemize}
...
\frametitle{Optimizations}
			...
			\begin{itemize}
				...
				\begin{itemize}
				 \item SDF update, Pose Tracking and Marching Cubes
				 \item In Marching Cubes, creating a point cloud per thread and combining them at the end is necessary
				 \begin{itemize}
				 \item In Pose Tracking, similar approach needed to parallelize matrix computation for solving linear system
				 \end{itemize}
				 \end{itemize} 
...
\hline
m  & method &  max $\Delta$$\xi$ &  play rate & trans error& notes\\
\hline
...
256  & Point-To-Point& 0.001  & 0.1 & \textbf{0.057 m} & every (9th) pixel\\
\hline
256 & Point-To-Point&  & & 0.047 m & \pbox{20cm}{Paper: same \\params as our}\\
& & & & &     \\
512 & Point-To-Plane&  & & \textbf{0.041 m} & \pbox{20cm}{Paper: best \\params}\\
\hline
\end{tabular}
