...
\begin{columns}
		...
		%\vspace{-30pt}
			\begin{itemize}
			    ...
			    \item We search transformation from last camera position\\
			    given in twist coordinates $\xi = (\underbrace{v_1, v_2, v_3}_{\text{linear velocity}}, \underbrace{w_1, w_2, w_3}_{\text{angular velocity}})$
			    \item Minimize the squared distance  between point cloud and surface %$E = \sum\limits_\mathbf{x} \Psi_{R, t}()$  
		\end{itemize}
	\end{columns}
}
...
\begin{columns}
                ...
                %\vspace{-30pt}
                        \begin{itemize}
                            \item We search transformation from last camera position\\
                            given in twist coordinates $\xi = (\underbrace{v_1, v_2, v_3}_{\text{linear velocity}}, \underbrace{w_1, w_2, w_3}_{\text{angular velocity}})$
                            \item Minimize the squared distance  between point cloud and surface
...
256  & Point-To-Point& 0.001  & 0.1 & \textbf{0.057 m} & (every pixel)\\
\hline
